 ...
 * 
 */
public class MockEvaluationStrategyManager {
    ...
    private static HashMap<UCMNavMultiPageEditor, MockEvaluationStrategyManager> strategyManagerInstances = null;
    // just in case we're actually accessing it via some non UI thread during export and/or before the app loads.
    private static MockEvaluationStrategyManager noEditorStrategyManagerInstance = null;
    ...
    public static synchronized MockEvaluationStrategyManager getInstance(UCMNavMultiPageEditor multieditor, boolean canRefresh) {

        if (strategyManagerInstances == null) {
            strategyManagerInstances = new HashMap<UCMNavMultiPageEditor, MockEvaluationStrategyManager>();
        }

        MockEvaluationStrategyManager soleInstance = null;
        ...
        if (multieditor != null) {
            if (strategyManagerInstances.get(multieditor) == null)
                strategyManagerInstances.put(multieditor, new MockEvaluationStrategyManager());
            ...
            if (noEditorStrategyManagerInstance == null)
                noEditorStrategyManagerInstance = new MockEvaluationStrategyManager();
    ...
    public static synchronized MockEvaluationStrategyManager getInstance(UCMNavMultiPageEditor multieditor) {
        return getInstance(multieditor, true);
    }

    public static synchronized MockEvaluationStrategyManager getInstance(boolean canRefresh) {
        return getInstance(null, canRefresh);
    }

    public static synchronized MockEvaluationStrategyManager getInstance() {
     ...
     * 
     */
    private MockEvaluationStrategyManager() {
...
//            calculateEvaluation();
    ...
    public synchronized void startDifferenceMode(EvaluationStrategy strategy) {
        ...
        for (Iterator iter = evaluations.keySet().iterator(); iter.hasNext();) {
            ...
            MetadataHelper.addMetaData(urnSpec, ie, Messages.getString("MockEvaluationStrategyManager.Evaluation"), Integer.toString(eval.getEvaluation())); // add temporary metadata for current evaluation //$NON-NLS-1$
        }
        ...
        bars.getStatusLineManager().setMessage(Messages.getString("MockEvaluationStrategyManager.StrategyDifferenceModeBaseStrategy") + strategy1.getName() + "\""); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    public synchronized void stopDifferenceMode() {
        ...
        if (comparisonEvaluations != null) {
            ...
            for (IntentionalElement ie : comparisonEvaluations.keySet()) {
                MetadataHelper.removeMetaData(ie, Messages.getString("MockEvaluationStrategyManager.Evaluation")); //$NON-NLS-1$
            }
